    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const responce: any = await axios.post(
          `${import.meta.env.VITE_WORKFLOW_BACKEND}/CreateWorkFlow`,
          {
            name: `${email + new Date()}`,
            email: email,
          },
  ...
  static getWorkFlow(): Promise<void> {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const responce: any = await axios.post(
          `${import.meta.env.VITE_WORKFLOW_BACKEND}/getAllworkFlow`,
          {
            email: email,
          },
          {
        ...
        sessionStorage.setItem(countWorkflow, `${responce.data.totalWorkflows}`);
        sessionStorage.setItem(totalStaps, `${responce.data.totalStaps}`);
        sessionStorage.setItem(totalStapsRun, `${responce.data.totalStapsRun}`);
        sessionStorage.setItem(totalTriggers, `${responce.data.totalTriggers}`);
